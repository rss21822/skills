...
実プロジェクト（正本30本・約9,000行・独立照合62巡・3系統監査）で実際に検出された欠陥23種を、再発順・原因別に整理したもの。各項目は **症状 / 実例 / 検出方法 / 修正型**。
...
### F-1. 未確認の製品判断が正本になる ★巻き戻し最大

- **症状**: intake で回答が無かった項目に AI が提案値を出し、その提案が確認を取られないまま GDD 本文に書かれ、下流の約30文書がそれを前提に完成する。
- **なぜ起きるか**: 提案を出すこと自体は正しい振る舞いなので、書き手にも読み手にも違和感がない。`[PROPOSAL]` marker が付いていないと、確認済みの判断と区別が付かなくなる。
- **検出**: 機械検出は困難。**予防が唯一の手段**——未確認の判断には必ず `[PROPOSAL]` を付け、承認ゲートで 0 にする。
- **修正型**: GDD は体系で唯一、内容が人間の所有物である。AI がやってよいのは「質問の提示」「未回答への `[PROPOSAL]` 提案」「確認済みのものの固定」の3つだけ、と handoff に明記する。`templates/gdd_handoff.md` の「製品判断の扱い」節。
...
- **修正型**: **GDD 執筆時に trigger 該当機能を列挙する**。結果に依存する判断は `[OPEN blocking: yes]` として登録し、理由を実体条件で書く（「FR-3.9 に Result が記録され Owner 承認されたとき closure」）。「たぶんこうなる」で下流を書かない。
- **注意**: 閾値は**計測前に**承認して固定する。データ取得後の閾値変更は判定の正当性を損なう。実プロジェクトでは事後変更を2回行い、再実験枠を使い切った。
