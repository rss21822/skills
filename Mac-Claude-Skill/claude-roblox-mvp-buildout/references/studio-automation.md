...
- OS入力、全画面capture、process close、desktop session切替は別々に承認を取る。強制終了と`tscon`は自動実行しない。
...
通常は現在版で確認したshortcutを使う。default経路でもOS入力承認が必要である。
...
OS入力前はauthoritative rosterを再照会し、`server PID → TargetPid → client:N → player ID/name`、join event ID、観測時刻、artifact SHAを保存する。session helperはprocess roleを検証するがplayer参加は証明しない。対応を現在状態から確認できなければ`BLOCKED`とする。
...
process closeの個別承認後だけ実行する。自動経路は`-Force`を必須とし、これ自体を非対話実行の明示assertionとして証跡へ残す。対話`-Confirm`はこの自動経路では使わない。
