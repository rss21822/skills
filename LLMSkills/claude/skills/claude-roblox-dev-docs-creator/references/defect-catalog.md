...
- **実例**: 3つの FR（モバイル騎乗操作・サーバー権威の高速 PvP・Multi-Place topology）のうち PASS したのは1つだけ。残り2つが未計測のまま**D5のW0引渡しゲートを止めている**。D5は実装side effectの開始承認ではない。下流文書は既に完成しているため、不成立なら大規模な是正になる。
