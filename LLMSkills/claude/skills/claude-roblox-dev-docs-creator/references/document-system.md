...
handoff、D1.5/D4 runtime provenance、D4 capsule/request/raw report/post-run attestation、preflight log、人間approval challenge/trusted transcript/statement/capture/external provenance/machine gate record、candidate/baseline manifest、snapshot bytes、post-sync manifest、W0 package は**製品正本inventoryの外**。それぞれ transition/audit/package の外側hashで束縛し、docs manifestへ混ぜない。人間向け決定意味の正本は`DECISIONS.md`、machine chainは同じID・対象・scopeへ解決する証跡であって別の製品判断を所有しない。baseline/post-sync の file bytes・sha256・file-set hash は lifecycle manifest / transition evidence が所有し、docs manifest に二重保持しない。

W0のoperator外部trust chain（verifier config／trust anchor、launch challenge、PREPARE execution attestation、prelaunch assertion、postexecution attestationと各detached signature、run-authorization challenge／presentation／transcript／statement／capture／provenance／authorization、run-admission attestation／signature、ADMIT execution-worker-ready receipt／signature）はproject・installed creator/receiver skill・prepared temp・W0 package外に置く。receiverはproject成果物へコピーせず、外部authorityがPREPARE process生成前からVALIDATE→run authorization→ADMIT semantic PASS→receipt検証→bootstrap PASS→first-effect capability消費までproject/source/tempのidentity lockを無停止維持する。artifact内のauthority/adapter/key文字列やlocal hashはtrust sourceにならない。ADMIT前にactual worker loadを主張せず、semantic PASS後だけtoken消費→suspended/pre-entry launch→actual closure照合を行う。receiptは現在のB2/package/WP bytes、closed path/worker/transfer/operation scope、未開始side effect、2回目validator実行、未使用worker-ready capabilityを署名し、bootstrapがnative検証する。失敗時はW0を再実行するか停止する。
