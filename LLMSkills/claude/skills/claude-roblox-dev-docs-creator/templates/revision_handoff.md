...
| 承認根拠 | `DECISIONS.md` の {人間 `[DECISION]` ID／`[AI-APPROVED]` ID + 委任 `[DECISION]` ID}。**選択内訳を本 handoff へ複製しない。** 作業前に記録と CR §{改訂 inventory の節} を読む |
...
- formal documentの `Status`／`Last approved` はP0中に変更しない。`[AI-APPROVED]` はD5の代替ではない
