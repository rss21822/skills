...
### {Critical|Major} D4-{TRACK}-{NNN} — {見出し}
...
- 新規数値なし（識別子・field 名・token は数値ではないので可）。実測由来の値を足す場合は evidence パスを添える
- **既存 `[DECISION]` の出所を落とさない。** 是正で行を書き換える際に出所欄が消えるのが典型
- Status は `Draft` を維持
...
通常handoffと同じworker、`transferApproval`、Class B context/response envelope、execution attestationを使う。Class Bへbaseline/照合記録をpathだけで渡さず、hash付きbundle全文へ含める。

**是正は初稿と同じ worker へ出す。** 途中で替えると、指摘の解釈が前提ごと変わる。替えるなら `DECISIONS.md` へ理由を残す。
...
```powershell
Copy-Item -LiteralPath "{target}" -Destination "docs\handoffs\out\{ID}_pre_correction_{filename}" -ErrorAction Stop
```
